...
def test_llm_agent_publishes_canonical_llm_dependency_type():
    entry = AgentRegistry.get_agent(f"{LLMAgent.__module__}.{LLMAgent.__name__}")
    assert entry is not None

    llm = next(dependency for dependency in entry.agent_dependencies if dependency.dependency_key == "llm")
    assert llm.required_type == "rustic_ai.core.guild.agent_ext.depends.llm.llm.LLM"
